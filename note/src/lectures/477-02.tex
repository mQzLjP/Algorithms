...
\begin{problem}[Secretary Problem]
...
\begin{problem}[Ski Rental Problem]
...
\begin{itemize}
	\item If we choose $k = B/2$:
	      \[ \text{ratio} = \max\frac{\ALG}{\OPT} = \max\begin{cases}
			      \frac{T}{T} = 1                                                                             & \text{if } T < B/2        \\
			      \max\frac{B/2 - 1 + B}{T} = \max\frac{3B/2 - 1}{T} = \frac{3B/2 - 1}{B/2} = 3 - \frac{2}{B} & \text{if } B/2 \leq T < B \\
...
\begin{proof}
	...
	Note that the adversary can always choose $T$ to be $\epsilon B$ (it turns out that such naive choice is sufficient for this proof.)

	In such case, we have:

	\[ \text{ratio} = \max\frac{\ALG}{\OPT} = \frac{\epsilon B - 1 + B}{\min(\epsilon B, B)} = \begin{cases}
...
\begin{definition}[Expected Competitive Ratio]
	The expected competitive ratio is $c$, if the expected cost incurred by the online \textbf{randomized} algorithm is at most $c$ times the cost incurred by the optimal offline algorithm, i.e. $\frac{\mathbb{E}[\ALG]}{\OPT} \leq c$.
...
\begin{eg}
	...
	\[ \text{expected ratio} = \frac{\mathbb{E}[\ALG]}{\OPT} = \begin{cases}
			1                                                                                     & \text{if } T < B/2        \\
