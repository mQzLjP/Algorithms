...
For sparse graphs ($m \ll n^2$), we tend to represent them as adjacency lists, but for dense graphs ($m\approx n^2$), we can represent them as adjacency matrices. For these graphs, we might want to do better than $O(mn) = O(n^3)$ APSP, so using adjacency matrices and matrix multiplication might be a good idea.
...
\begin{problem}[APSP for Unweighted, Undirected Graphs]
