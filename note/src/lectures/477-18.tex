...
The implement of Galler-Fischer Union-Find Algorithm is easy. Each element keeps a \textbf{parent pointer} and a \textbf{rank}. Consider each set $A\in \mathcal P$ is a rooted tree, where the root has a pointer to $\text{rep}(A)$. Initially, $\rank(v) = 0$ and $\text{parent}(v) = \text{NULL}$ for all $v$.
