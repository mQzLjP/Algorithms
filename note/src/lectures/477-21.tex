...
\begin{proof}
	...
	We can assume $m > k^2$, otherwise we can find a $k$-cut by other algorithms. Thus, the probability above at least $\frac{1}{8k}$. By repeating for $10k\log n$ times, we can find a $k$-cut with probability $1 - N^{-1}$.
...
\begin{theorem}
	The running time of Algorithm~\ref{alg:vertex-cut} is $\tilde{O}(k^2 m)$.
\end{theorem}
