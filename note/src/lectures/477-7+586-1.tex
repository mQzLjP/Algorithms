...
\begin{itemize}
	...
	\item evaluation: $A(x) = (\cdots((a_{n-1}x + a_{n-2})x + a_{n-3})x + \cdots ) x + a_0$ (Horner's method).
...
\begin{algorithm}[H]
	\KwIn{$(a_0, a_1, \ldots, a_{n-1}, \omega_n)$ where \( A(x) = \sum_{i=0}^{n-1} a_i x^i \) and \( n \) is a power of \( 2 \).}
	\KwOut{Point-value representation of \( A(x) \) at \( n \)-th roots of unity.}
	\If{\( n = 1 \)}{
		\Return{\( \{(1, a_0)\} \)}\;
	}
	Compute \( A_{\text{even}}(x) \) and \( A_{\text{odd}}(x) \)\;
	Recursively compute $\text{Even} \gets \text{FFT}(A_{\text{even}}, \omega_n^2)$ and $\text{Odd} \gets \text{FFT}(A_{\text{odd}}, \omega_n^2)$\;
